...
class SyncEngine {
  ...
  private setStatus(partial: Partial<SyncStatus>): void {
    ...
    this.status.pending = getDirtyNotes().length
  ...
  private async fetchVaultTree(rootId: string): Promise<{ folders: RemoteItem[]; files: RemoteItem[] }> {
    const drive = await this.drive()
  ...
  private async reconcileRemoteTree(
    rootId: string,
    tree: { folders: RemoteItem[]; files: RemoteItem[] }
  ): Promise<void> {
