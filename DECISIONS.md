...
### D22 — Plugin platform (2026-09-14, v0.11.0)
The plugin era: research, review, and the new Fountain screenplay module ship as **bundled plugins** mounted through a generic mode registry; third-party plugins load from `userData/plugins/<id>` via manifest.json `{id, name, version, entry, permissions[ui|notes|storage|ai|files]}`.
- **Two trust tiers**: bundled (in-repo registrars, full module access, toggleable not removable) and third-party (code evaluated in a restricted `new Function` scope with only the SDK passed; code fetched over IPC from the main process — the renderer never touches the plugin dir).
- **SDK facets** (`HalPluginSdk`): ui (registerMode returning a cleanup fn, addStyle, status), notes (list/open/create/save/search), storage (per-plugin KV in `plugin_storage`, migration v6), ai (router-backed chat + streaming via reqId events, gated), files (saveText export dialog). Facets are null unless the manifest's permissions grant them — bundled get all.
- **Enable/disable everywhere**: state in settings.plugins.enabled (bundled default on, third-party default off); Settings → Plugins tab shows toggles, install-from-folder, remove; the rail renders modes from the registry and falls back to Notes if the active mode's plugin is disabled.
- **Screenplay module**: fountain parser (pure, 8 tests) — scene headings, characters, parentheticals, dialogue, transitions, sections/synopses/notes/page-breaks; CodeMirror line-decorator highlighting; character/scene autocomplete; scene outline rail; scene/page/character stats; screenplays are plain notes (registry of note ids in plugin KV) so they sync to Drive for free; `.fountain` export via native save dialog.
- **AI as a toggleable capability** (the deferred D19 policy): `ai/gate.ts` — no registered key means no AI surface anywhere; with a key, one master switch in Settings → Integrations. Capture, research engine, review cards, and plugin AI all route through it.
Deferred to the public-SDK release (next): npm package, scaffold generator, dev harness, versioned compat surface. Multi-select bulk ops remain deferred (D18).
...
- **I13 — The 1.0-day folder-ejection bug (2026-09-14)**: a debounced note push (2s after typing) could fire before its parent folder was ever uploaded; the upload fell back to the vault root, and the next reconcile then moved the note out of its folder locally — the user saw sync stripping notes from folders. Fix:  walks the ancestor chain and uploads any still-local folders (bottom-up) before the note's upload. Lesson: **any push that resolves a parent must guarantee the parent exists remotely first**; a fallback-to-root on a missing parent is silent data displacement, not a safe default.
- **I14 — The regex that wasn't (2026-09-14, v1.0.10)**: the horizontal-rule regex in the live-preview editor contained a literal  control character where the  backreference should have been — a Python escape-corruption during a patch script. It silently matched nothing across three shipped releases while passing typecheck. Fixed by byte-level replacement, verified with cat -A before shipping. Lesson (reinforcing I13): **scripted patches need byte-level output verification**, not just the type checker — a control character in a regex is invisible to every tool that isn't looking for it.
- **I15 — The folder-ejection double (2026-09-14, v1.0.1–v1.0.2)**: two related bugs in one day. (a) A debounced note push could fire before its parent folder was uploaded → note fell back to vault root → reconcile then "moved" it out of its folder locally. (b) The reconcile pass itself walked Drive before pushing local moves, treating remote state as authoritative and reverting user drags. Fixes: ensureParentPushed (recursive ancestor upload) and a guard on the remote-move branch that only fires when the local parent matches the last-synced parent. Lesson: **reconcile-then-push ordering must account for pending local mutations**, or the reconcile undoes the user's intent.
